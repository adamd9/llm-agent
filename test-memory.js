// Test script for memory system with delimiters
const memory = require('./src/core/memory');
const logger = require('./src/utils/logger');
const fs = require('fs');
const path = require('path');

async function testMemoryDelimiters() {
  console.log('Testing memory system with delimiters...');
...
\`\`\``;
  ...
  const longTermData = `This is a test of long-term memory with multi-line content.
It should be properly delimited with <MEMORY_CONTENT> and </MEMORY_CONTENT> tags.
...
- Bullet point 2
  ...
  if (fs.existsSync(longTermPath)) {
    ...
    console.log('Long-term memory file content:');
    console.log(longTermContent);
    
    // Parse the long-term memory content
    console.log('\n6. Parsing long-term memory content...');
    const parsedLongTermMemories = memory.parseMemoryContent(longTermContent);
  ...
  const anotherMemory = `This is another test memory specifically about multi-line content.
It contains information about formatting and delimiters.
The new memory system uses <MEMORY_CONTENT> and </MEMORY_CONTENT> tags to clearly
define the boundaries of memory content, making it easier for both humans and LLMs
to understand where memory content begins and ends.`;
  ...
  try {
    const longTermResult = await memory.retrieveLongTerm('ego', 'multi-line content');
...
// Run the test
testMemoryDelimiters().catch(err => {
